# choosing to not configure the test to work any further with the database
# even with a disconnect in the teardown, this keeps throwing flags that there are too many connections to the DB
# and will stop after 20 or so tests
# some quick googling revealed that you generally want to keep your DB out of your tests
# since the test should be encapsulated
# so making this work for the sake of making it work also seems to be against the
# spirit of how some of this testing is done in the first place
...
class ContactTest < Minitest::Test
  ...
  def teardown
    # FileUtils.rm_rf(@storage.contacts_path) if @storage
    @storage.disconnect if @storage
  end

  def session
    current_session = last_request.env['rack.session']
    @storage = DatabasePersistence.new(current_session)
    current_session
  end

  def user_session
    { 'rack.session' => { user: 'testuser' } }
  end

  def second_user_session
    { 'rack.session' => { user: 'testuser2' } } 
  end
